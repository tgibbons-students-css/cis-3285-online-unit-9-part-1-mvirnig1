...
return tradeData;
        ...
        private bool ValidateTradeData(string[] fields, int currentLine)
        {
            ...
            if (tradeAmount < 1000)
            {
                LogMessage("WARN", "Trade anount on line {0} is too small: '{1}'", currentLine, fields[1]);
                return false;
            }
            if (tradeAmount > 100000)
            {
                LogMessage("WARN", "Trade anount on line {0} is too large: '{1}'", currentLine, fields[1]);
                return false;
            }
            decimal tradePrice;
        ...
        private void LogMessage(string msgType, string message, params object[] args)
        {
            ...
            {
                logfile.WriteLine("<log><msgType>"+msgType+"</type><message>" + message + "</message></log> ", args);
            }
        }
